...
import { FechamentoCulto, Lancamento, TipoLancamento, CategoriaEntrada, CategoriaSaida, FormaPagamento } from '../types';
...
export const LancamentosView: React.FC<LancamentosViewProps> = ({ fechamento, setFechamento }) => {
  ...
  const handleAddLancamento = (e: React.FormEvent) => {
    ...
    const newLancamento: Lancamento = {
      ...
      descricao: descricao.trim() || (tipo === 'entrada' ? 'Dízimo / Oferta / Doação' : 'Despesa / Saída'),
      valor,
      formaPagamento,
  ...
  const filteredLancamentos = fechamento.lancamentos.filter((l) => {
    const matchesSearch =
      l.descricao.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (l.nomePessoa && l.nomePessoa.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    const matchesFilter =
      ...
      (filterTipo === 'ofertas' && (l.categoria === 'oferta_culto' || l.categoria === 'oferta_missoes'));

    return matchesSearch && matchesFilter;
  });

  return (
    <div id="lancamentos-view-container" className="flex flex-col min-h-full bg-slate-950 text-slate-100 p-4 md:p-6 space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 md:p-6 shadow-xl space-y-5 max-w-5xl mx-auto w-full">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-100">Lançamento de Dízimos, Ofertas, Doações e Saídas</h2>
              <p className="text-xs text-slate-400">
                Data do registro gerada automaticamente pelo sistema a cada lançamento.
              </p>
            </div>
          </div>
        ...
        <div className="bg-slate-950 border border-slate-800/80 rounded-2xl p-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-300">
          <div className="flex items-center gap-2">
            ...
            <span>Registro da Transação: <strong className="text-slate-100">Data e Hora Automáticas</strong></span>
          </div>
        ...
        <form onSubmit={handleAddLancamento} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 max-w-md mx-auto">
            ...
            >
              ...
              <span>Saída (Despesa / Pagamento)</span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              >
                {tipo === 'entrada' ? (
                  <>
                    <option value="dizimo">Dízimo do Membro</option>
                    <option value="oferta_culto">Oferta do Culto</option>
                    <option value="oferta_missoes">Oferta de Missões</option>
                    <option value="oferta_especial">Oferta Especial / Projeto</option>
                    <option value="doacao">Doação Direta</option>
                  ...
                  <>
                    ...
                    <option value="internet">Conta de Internet / Telefone</option>
                    ...
                    <option value="material_ebd">Material de EBD / Infantil</option>
                    <option value="acao_social">Ação Social / Assistência</option>
            ...
            {/* Nome do Dizimista / Pessoa */}
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">
                {tipo === 'entrada' ? 'Nome do Dizimista (Opcional):' : 'Favorecido / Credor:'}
              </label>
              <input
                type="text"
                value={nomePessoa}
                onChange={(e) => setNomePessoa(e.target.value)}
                placeholder={tipo === 'entrada' ? 'Ex: Irmão João Silva' : 'Ex: Pr. Misael'}
          ...
          {/* Descrição Detalhada */}
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Descrição / Observação do Lançamento:</label>
            <input
              type="text"
              value={descricao}
          ...
          >
            ...
            <span>Confirmar Lançamento no Caixa do Culto</span>
          </button>
        </form>
      </div>
